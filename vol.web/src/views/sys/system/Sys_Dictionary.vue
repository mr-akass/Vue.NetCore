...
import {
  VOL_TAG_EP_SWATCH,
  VOL_TAG_PRESET_HEX,
} from "@/components/basic/VolTag/volTagColors.js";
function buildVolTagColorOptions() {
  const list = [];
  Object.keys(VOL_TAG_EP_SWATCH).forEach((k) => {
    list.push({
      value: k,
      label: k,
      swatch: VOL_TAG_EP_SWATCH[k],
      soft: false,
    });
  });
  Object.keys(VOL_TAG_PRESET_HEX).forEach((k) => {
    const hex = VOL_TAG_PRESET_HEX[k];
    list.push({
      value: k,
      label: k,
      swatch: hex,
...
const volTagColorOptions = buildVolTagColorOptions();

function volTagSwatchNode(hit) {
  const base = {
    width: "14px",
    height: "14px",
    borderRadius: "3px",
  ...
  if (hit.soft && hit.accent) {
    return (
      <span
        style={{
          ...base,
          backgroundColor: `color-mix(in srgb, ${hit.accent} 8%, #ffffff)`,
          border: `1px solid color-mix(in srgb, ${hit.accent} 30%, #ffffff)`,
        }}
      />
    );
  }
...
const onInited = async () => {
  ...
  const col = gridRef.detailOptions.columns.find((x) => {
    return x.field == "Color";
  });
  col.tip = {
    text: "表格字段显示的数据源背景颜色,设置后刷新表格查看效果",
  };
  col.edit = null;

  col.render = (h, { row, column, index, editInfo }) => {
    const hitForVal = (val) => {
      if (!val) return null;
      const s = String(val).trim();
      let hit = volTagColorOptions.find((o) => o.value === s);
      if (hit) return hit;
      if (/^#|^rgb/i.test(s)) return null;
      const m = s.match(/^(.+)(2)$/);
      if (!m) return null;
      const base = m[1].toLowerCase();
      const hex = VOL_TAG_PRESET_HEX[base];
      if (!hex) return null;
      return { value: s, label: s, accent: hex, soft: true };
    };

    /** 与下拉选中项 / option 行一致：左小方 + 右 type 文案 */
    const colorSummaryRow = (value) => {
      if (value === undefined || value === null || String(value).trim() === "") {
        return (
          <span style={{ color: "var(--el-text-color-placeholder)", fontSize: "13px" }}>
          {proxy.$ts('点击选择颜色')}
          </span>
        );
      }
      const hit = hitForVal(value);
      const text = value ?? "";
      if (!hit && value && /^#|^rgb/i.test(String(value).trim())) {
    ...
    const isEditRow = editInfo && editInfo.rowIndex === index;

    return isEditRow ? (
      <div style="width: 100%; min-width: 168px">
        <el-select
          v-model={row.Color}
          clearable={false}
          filterable
          ...
          style="width:120px"
          v-slots={{
            label: ({ value }) => colorSummaryRow(value),
          }}
        >
          {volTagColorOptions.map((o) => (
      ...
      <div
        onClick={(e) => {
          e.stopPropagation();
          const tb = gridRef.getTable && gridRef.getTable();
          if (tb && tb.edit) {
            tb.edit.rowIndex = index;
          }
        }}
...
const saveBefore = (formData) => {
  ...
  if (
    ...
    (editFormFields.DbSql.indexOf("value") == -1 ||
      editFormFields.DbSql.indexOf("key") == -1)
  ) {
    proxy.$message.error(
              ...
              return (
                <div>
                  <div style="font-size:12px;line-height:2">
                    1. 如果需要从数据库返回数据源sql为：SELECT key字段 as 'key',名称字段
                    as 'value' FROM table{" "}
                  </div>
                  <div style="font-size:12px;line-height:2">
                    2. 如果是级联,数据源sql为： SELECT key字段 AS id,父级id字段 as
                    parentId, key字段 AS 'key',名称字段 AS 'value' FROM table{" "}
                  </div>
                  <div style="font-size:12px;line-height:2">
                    3.
            ...
            reference: () => {
              return (
                <div style="color:#2f54eb">
                  <span class="el-icon-warning-outline"></span>数据源sql
                </div>
              );
            },
          }}
