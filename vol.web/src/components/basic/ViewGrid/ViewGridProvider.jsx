...
export const initOptions = (proxy, props, dataConfig) => {
  if (props.table.showFooterDetail) {
    dataConfig.showFooterDetail.value = true;
  }
  if (props.table.quickQueryFields) {
    dataConfig.queryFields.value = props.table.quickQueryFields.split(',')
  }
};
...
export const resetEditForm = (proxy, props, dataConfig, row) => {
  const isAdd = dataConfig.currentAction.value == action.ADD;
  //重置明细表
  resetDetailTable(proxy, props, dataConfig, row, isAdd);
  ...
  if (!props.resetEditFormBefore(row, isAdd, dataConfig.isCopyClick.value)) {
    return;
  }
  if (
    !proxy.resetEditFormBefore.call(
      proxy,
      row,
      isAdd,
  ...
  //console.log('resetEditFormBefore')

  proxy.base.resetForm(props.editFormFields, props.editFormOptions, row, false);

  props.resetEditFormAfter(row, isAdd, dataConfig.isCopyClick.value);
  proxy.resetEditFormAfter.call(
    proxy,
    row,
    isAdd,
    dataConfig.isCopyClick.value
  );
  //console.log('resetEditFormAfter')
...
const modelOpenBeforeAsync = async (proxy, props, dataConfig) => {
  ...
  res = await dataConfig.dyScript.modelOpenBeforeAsync?.(dataConfig.currentRow.value,
    dataConfig.currentAction.value,
    dataConfig.isCopyClick.value);
  if (res === false) {
    return false;
  ...
  res = await dataConfig.dyScript.modelOpenBefore?.(dataConfig.currentRow.value,
    dataConfig.currentAction.value,
    dataConfig.isCopyClick.value);
  if (res === false) {
    return false;
...
export const modelOpenProcess = (proxy, props, dataConfig, row) => {
  proxy.$nextTick(() => {
    // console.log('setFormAddOrUpdateReadonly')
    //设置新建\编辑时只读
    proxy.base.setFormAddOrUpdateReadonly(props.editFormOptions,dataConfig.currentAction.value)
    if (dataConfig.currentAction.value=='Add') {
          proxy.base.setFormDefaultValue(props.editFormOptions,props.editFormFields)
    }
    ...
    dataConfig.dyScript.modelOpenAfter?.(row,
      dataConfig.currentAction.value,
      dataConfig.isCopyClick.value)
    ...
    proxy.$nextTick(() => {
      proxy.$refs.form?.setOptionLayoutKey?.();
    });
  });
};
...
export const getUrl = (action, ingorPrefix, table, dyPage, props,asyncApi) => {
  //动态页面自定义接口
  // if (dyPage === true) {
  //   return "api/xxxx" + action;
  // }
  if (props && props.generic) {
    table = { url: '/generic/' }
  }
  //是否忽略前缀/  获取操作的url
  return (!ingorPrefix ? "/" : "") + "api" + table.url + action+(asyncApi?'Async':'');
};
...
export const initBoxHeightWidth = (proxy, props, ctx, dataConfig) => {
  ...
  if (
    ...
    editor.uploadImgUrl = getUrl("upload", null, props.table, null, props);
  ...
  if (detail.columns && detail.columns.length > 0) {
    ...
    detailOptions.url = getUrl("getDetailPage", null, props.table, null, props,dataConfig.asyncApi.value);
  } else {
    ...
    const clw = clientWidth * maxWidthRate;
    clientWidth = clw;
  ...
  if (!boxOptions.width) {
    if (props.details && props.details.length) {
      boxOptions.width = clientWidth + clientWidth * 0.1;
    } else {
      boxOptions.width = clientWidth + 30;
    }
  }
  const maxWidth = document.documentElement.clientWidth * 0.95;
  if (boxOptions.width > maxWidth) {
    boxOptions.width = maxWidth;
  }
  if (height <= 100) {
    height = 150;
  }
...
export const fullscreen = (full) => {
  //弹出框全屏方法
  proxy.fullscreen?.call(proxy, full);
  props.fullscreen?.(proxy, full);
  //获取弹出框高度计算明细表高度
  //gridRef.boxOptions.height
  //设置明细表高度
  //gridRef.getTable().realHeight=400;
};
