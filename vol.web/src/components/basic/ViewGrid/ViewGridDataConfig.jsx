...
const dataConfig = () => {
  return {
    asyncApi:ref(''),//启用异步接口
    tableV2: ref(false), //启用高性表格(解决大批量数据显示或者编辑)
    rowHeight: ref(rowHeight),
    eventNext: ref(true), //编辑表单按回车后是否跳转下一个输入框
    dyScript: {}, //动态脚本
    dyPage: false, //动态页面
    defaultFormReadonlyFields: ref([]), //默认表单只读的字段(审批设置只读与还源的时候需要使用)
    // 是否弹出框审批
    ...
    detailOptions: reactive({
      tableV2: false, //启用高性表格(解决大批量数据显示或者编辑)
      rowHeight: rowHeight,
      // 明细表隐藏分页
    ...
    sortable: ref(false),
    extraHeight: ref(0),
    showTabs: ref(true), //一对多明细表是否显示tab标签，false会将明细表从上往下显示
    orginColumnFields: reactive([]),
    //是否审批点击
    isAuditClick: ref(false),
    full: ref(false), //编辑弹出框默认是否最大化
    gridBodyText:ref(''),
    modelBodyText:ref(''),
    showFooterDetail:ref(false) //主表列表下面是否显示明细表数据
  };
};
