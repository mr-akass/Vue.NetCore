let serviceFilter = {
  ...
  mounted() { },
  async showAdvancedSearchBefore() {//高级查询显示前
    return true;
  },
  searchBefore(param) {
  ...
  delRowBefore(rows, table, item, index) {
    //弹出框删除明细表的行数据(只是对table操作，并没有操作后台)
    return true;
  },
  delRowAfter(rows, table, item, index) {
    //弹出框明细表删除后方法(只是对table操作，并没有操作后台)
    return true;
  },
  saveConfirm(callback, formData, isAdd) {
    //保存前确认操作，formData表数据，isAdd是否新建操作
    //保存前自定义确认操作
    callback && callback();
  },
  addBefore(formData, isCopyClick) {
    //新建保存前formData为对象，包括明细表
    return true;
  },
  async addBeforeAsync(formData, isCopyClick) {
    //异步处理,功能同上(2020.12.06)
    return true;
  },
  addAfter(result, isCopyClick) {
  ...
  async submitBefore(formData, isAdd, isCopyClick) {
    return true;
  },
  async submitAfter(result, formData, isAdd, isCopyClick) {
    return true;
  },
  auditBefore(ids, rows, callback) {
    //审核前
    return true;
  },
  ...
  cancelAuditBefore(rows,isCancel){
    //流程撤销前
    return true
  },
  cancelAuditAfter(result, rows,isCancel) {
    //取消流程后
    return true;
  },
  urgentAuditAfter(result, rows) {
    //崔办后
    return true;
  },
  antiAfter(result, param, rows, file) {
    //反审后
    return true;
  },
  resetEditFormBefore(row, isAdd, isCopyClick) {
    //重置新建表单前的内容
    return true;
  },
  resetEditFormAfter(row, isAdd, isCopyClick) {
    //重置新建表单后的内容
    return true;
  },
  modelOpenBefore(row) {
  ...
  importAfter(data) {
    ...
    importDetailBefore(formData,callback) {//明细表导入前
    return true;
  },
  importDetailAfter(rows) {
    return true;
  }, //明细表导入后方法
  //添加导入前的方法
  ...
  detailSelectable(row, index, item) {
    return true;
  }, //明细表checkbox复选框是否可以选中
  ...
  getAuditTable(rows, isAnti, view) {
    //获取实际审批的表
    return "";
  },
  ...
  searchSubDetailBefore(rows, table, item) {
    return true;
  }, //三级明细表加载前
  ...
  detailHeaderDragend({ newWidth, oldWidth, column, event }, table) {//明细表拖动表头改变了列的宽度事件
  },
  headerDragend({ newWidth, oldWidth, column, event }) {//主表拖动表头改变了列的宽度事件
  },
  fullscreen(full) {
    return true;
  }
