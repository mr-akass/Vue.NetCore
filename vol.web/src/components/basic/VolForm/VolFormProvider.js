...
export const initDicData = async ({ proxy, formOptions, select2Count = 3000, resetData, callback }) => {
  await initDataSource(formOptions, proxy.http, proxy.$ts, select2Count, resetData, callback);
}
...
export const initDataSource = async (
  ...
  await http.post('api/Sys_Dictionary/GetVueDictionary', dicKeys, false).then((dicData) => {
...
//获取图片只读样式
export const getImgReadonlyStyle = (item) => {
  if (item.style) {
    return item.style;
  }
  if (item.imgOption) {
    return {height:(item.imgOption.height||100)+'px',width:(item.imgOption.width||100)+'px'}
  }
}