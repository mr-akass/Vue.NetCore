const colors = [null, "success", "warning", "danger", "info"];
...
export const getCellColor = (row, column, formatConfig) => {
  ...
  const item= column.bind?.data?.find(x=>{return x.key==val});
  if (item?.color) {
     return item.color;
  }
...
export const cellFormatter = (proxy, row, column, template) => {
  ...
  if (
    ...
    proxy.$global.table.cascaderFullPath !== false
  ) {
  ...
  if (column.edit && editType.includes(column.edit.type)) {
    if (column.edit.type == "treeSelect" && column.multiple === false) {
