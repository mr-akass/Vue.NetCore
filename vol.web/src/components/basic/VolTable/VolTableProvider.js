...
export const getColumnFilters = (proxy, column, tableData) => {
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    if (
      ...
      let obj = {
        text: cellFormatter(proxy, row, column, true),
...
export const initSortable = (props, emit, nextTick, refTable, getTable) => {
  ...
  Sortable.create(tbody, {
    ...
    onEnd: ({ newIndex, oldIndex }) => {
      const { rows, elTableRef, scrollInfo } = getTable(); //  tableData
      // elTableRef.setScrollTop(200);
      // return;
      ...
      nextTick(() => {
        ...
        nextTick(() => {
          if (scrollInfo.scrollTop || scrollInfo.scrollLeft) {
            elTableRef.setScrollTop(scrollInfo.scrollTop);
            elTableRef.setScrollLeft(scrollInfo.scrollLeft);
          }
        });
      });
    },
...
export const validateRow = (proxy, props, row, column) => {
  ...
  if (!validateColum(proxy, props, column, row)) {
    ...
    proxy.$message.error(
      proxy.$tst("第[{$ts}]行", row.elementIndex + 1) +
        `,[${proxy.$ts(column.title)}]` +
        proxy.$ts(proxy._errMsg || "是必填项目")
    );
    return false;
  }
  proxy.errField = "";
...
export const validateColum = (proxy, props, option, data) => {
  ...
  if (option.hidden || option.readonly) return true;
  let val = data[option.field];
  if (option.require || option.required) {
    if (proxy.base.isEmptyValue(val)) {
      if (!proxy.errorFiled) {
...
export const delTableRow = (proxy, edit, tableData, rows, indexArr) => {
  if (rows.length == 0)
    return proxy.$message.error(proxy.$ts("请选择要删除的行!"));
