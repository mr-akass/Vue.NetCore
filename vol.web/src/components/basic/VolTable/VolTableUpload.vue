  ...
  <VolBox v-model="model" title="上传" :height="228" :width="500" :padding="15" lazy>
    ...
    <template #footer>
      <div style="text-align: center">
        <el-button type="default" size="small" @click="model = false">{{ $ts('关闭') }}</el-button>
...
const fileClick = (index, file, files) => {
  ...
  return column.value.fileClick(index, file, files,row)
}
...
const removeBefore = (index, file, files) => {
  ...
  return column.value.removeBefore(index, file, files,row)
}

const saveUpload = () => {
  column.value.saveUpload?.(fileInfo.value, row.value,row)
  //生成保存后返回的路径
...
const initUploadInfo = (editRow, editColumn, url) => {
  ...
  if (editColumn.edit.url === undefined) {
    ...
    uploadUrl.value = `api/${tb}/upload${proxy.base.getAsyncApi(tb)}`
  } else {
