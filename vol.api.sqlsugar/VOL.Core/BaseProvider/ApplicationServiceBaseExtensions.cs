...
namespace VOL.Core.BaseProvider
{
    public static class ApplicationServiceBaseExtensions
    {
        ...
        {
            type = GetActionLogEntityType(type);
            string table = type.GetEntityTableName();
            return TableColumnContext.TableInfo.Any(x => x.TableTrueName == table && x.ActionLog == 1);
        }
        ...
        {
            return queryable.GetISugarQueryableOrderBy(orderBySelector);
        }
    }
}
