...
### 18. 主题个性化（2026-09-01，对应报告第 21 节；背景图铺满/折叠重叠修复见报告第 23 节，圆角横条修复见第 24 节，搜索条/工具栏叠色修复见第 26 节，[恢复默认]/[恢复上次] 见第 27 节）
...
- **面板底部五个按钮，语义必须分清（报告第 27 节）**：[保存] 落库；**[恢复默认]** 只把旋钮赋成
  `DEFAULT_THEME` 并实时预览、**不请求服务端**（因此不做二次确认——点 [恢复上次] 就能原样拿回来），
  背景图跟着清但文件要等用户点 [保存] 时才由后端顺手删；**[恢复上次]** 是**重新拉服务端**那份
  （不能用打开面板时的内存快照：背景图"点上传就落库"且会删掉上一张，用旧快照会指向已删除的文件）；
  [重置] 才是不可逆的（删记录+删图+刷新回框架原生）。`themeManager.loadTheme` 的返回值是**三态**——
  主题对象 / `null`(服务端确实没配置) / `undefined`(请求失败)，[恢复上次] 靠这个区别决定"清成默认"还是"只提示失败"。
